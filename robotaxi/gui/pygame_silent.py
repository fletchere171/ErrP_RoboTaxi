...
class TiDReceiver(threading.Thread):
    ...
    Produces tuples: (prob:float, t:float)
    """
    ...
    def run(self):
        ...
        while not self._stop.is_set():
            ...
            while self.bci.idStreamer_bus.Has("<tobiid", "/>"):
                ...
                try:
                    prob = float(self.bci.id_msg_bus.GetEvent())
...
class PyGameGUI:
    ...
    def drain_tid_events(self):
        ...
        Each item is (prob:float, t:float), where prob is the error probability from decoder in [0,1].
        """
        events = []
        try:
            while True:
